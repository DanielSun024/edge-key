<template>
  <div class="min-h-screen relative pb-20 font-sans selection:bg-white/30">
    ...
    <div class="relative z-10 max-w-6xl mx-auto px-4 pt-2 space-y-6">
      ...
      <!-- 分类切换：透明背景 -->
      <section class="rounded-2xl border border-white/10 bg-transparent p-2 flex items-center gap-2 overflow-hidden">
      ...
      <div v-if="filteredProducts.length" class="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        <article 
          ...
          :key="product.id" 
          class="group relative bg-transparent rounded-3xl border border-white/20 p-5 flex items-center gap-5 hover:border-white/50 hover:-translate-y-1 transition-all duration-500 hover:bg-white/5"
        >
          <!-- 图片容器 -->
      ...
      <!-- 空状态 -->
      <div v-else class="py-32 text-center bg-transparent rounded-[3rem] border border-dashed border-white/20 text-white/50 flex flex-col items-center gap-4">
